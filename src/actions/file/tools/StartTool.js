    ...
    if (
      ...
      if (file.selectionCanvas && file.selectedTool.id !== "select") {
        ...
        if (!file.toolSelectionCanvas)
          file.toolSelectionCanvas = new Canvas(
            null,
            canvas.width,
            canvas.height
          );
      ...
      await file.selectedTool.start(
        file,
        editCanvas,
        x / file.zoom,
        y / file.zoom,
        file.toolCanvas,
        file.selectedTool.size
      );

      // run first persistence if needed
      if (file.selectedTool.persistent)
        file.selectedTool.persist(file.toolCanvas, file, true);
